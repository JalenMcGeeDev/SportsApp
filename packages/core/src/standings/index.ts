...
export type Standing = { registrationId: string; played: number; wins: number; losses: number; ties: number; points: number; scored: number; conceded: number; difference: number; discipline: number; rank: number; trace: string[]; unresolved: boolean };
...
export function computeStandings(ids: string[], games: Game[], rules: Rules, seed = 42): Standing[] {
  ...
  const rows = ids.map((registrationId): Standing => {
    const row: Standing = { registrationId, played: 0, wins: 0, losses: 0, ties: 0, points: 0, scored: 0, conceded: 0, difference: 0, discipline: 0, rank: 0, trace: [], unresolved: false };
    for (const game of completed) {
      ...
      if (scored > conceded) { row.wins++; row.points += game.status === "forfeit" ? rules.forfeitPoints : rules.winPoints; }
      else if (scored < conceded) { row.losses++; row.points += rules.lossPoints; }
      else { row.ties++; row.points += rules.tiePoints; }
      if (conceded === 0 && scored > 0 && game.status !== "forfeit") row.points += rules.shutoutPoints;
    }
    return row;
  });
