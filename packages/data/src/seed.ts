...
export function createDemo(): Workspace {
  ...
  const registrations: Registration[] = names.map((teamName, index) => ({
    ...
    refundedCents: 0, platformFeeCents: 0, stripePaymentIntentId: null, stripeRefundId: null, stripeCustomerId: null, stripeInvoiceId: null, invoiceSentAt: null, waitlistPosition: null, rosterApproved: index % 4 !== 2,
  ...
  const tournament: Tournament = {
    ...
    divisions, registrations, games: [], publishedGames: [], invitees: [], invitesSentAt: now, registrationInvitesSentAt: now, followers: [],
    contacts: registrations.filter((registration) => registration.status === "accepted").map((registration) => ({ id: `contact-${registration.id}`, name: registration.coachName, email: registration.coachEmail, note: `${registration.teamName} - Coach` })),
  ...
  return workspaceSchema.parse({
    ...
    tournaments: [tournament, upcoming], announcements: [{ id: "welcome", tournamentId: tournament.id, subject: "Welcome to the Autumn Invitational", body: "Team check-in opens at 7:30 AM at the tournament desk. Please have your approved roster ready.", contactIds: [], sentAt: "2026-09-25T20:00:00.000Z", recipientCount: 12 }],
